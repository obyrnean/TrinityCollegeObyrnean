...
void add_games(Game game) {
     if (yearCount >= MAX_YEARS) {
        printf("Error: Maximum number of years reached. Cannot add more games.\n");
        return;
    }
...
void printGames() { //sort and print top 5 games for each year
    for (int i = 0; i < yearCount; i++) {
        quickSort(year[i].games, year[i].count); //sort the games for the year

        printf("Top five games for year %d:\n", year[i].year);
        for (int j = 0; j < 5 && j < year[i].count; j++) { //print top 5 games of the year
            int num = j + 1; 
            printf("%d. %s.\n", num, year[i].games[j].title);
        }
        printf("\n");
    }
}
