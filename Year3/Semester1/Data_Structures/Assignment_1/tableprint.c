...
int hash_function(char* s){ //generates a slot number for each name (s) wanted to insert into the hash table
    int hash = 0;
...
Element* search (char* name){ //searches and returns the element with name name or NULL if the element is not present
    int slot = hash_function(name); //get the slot number of the input name
...
void insert(char* name){ //add element at the correct place in the list
    int slot = hash_function(name); //get the slot number where we're trying to fit the input name
    int startingSlot = slot; //retain the slot we started on
...
int main(int argc, char *argv[]){
    ...
    /*    printf("%s", NAME_PROMPT); //print user invitattionsA
        scanf("%s", &name); //read the name user is sarching for
        while (strcmp(name, "quit") != 0) { //if user doesn't type quit, print if searched name is in table and its frequency
           ...
           scanf("%s", &name); //read the next name user is sarching for
        }*/
       printHashTable();
